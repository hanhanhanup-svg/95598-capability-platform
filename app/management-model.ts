...
export type Relation={id:string;from:string;to:string;type:string;targetLevels?:number[];requirements?:{required:boolean;thresholdType:string}[];behaviors?:string[]};
export type Handoff={id:string;scenarioId:string;taskId:string;leadRoleId:string;supportRoleIds:string[];reviewRoleId:string;escalateRoleId:string;materials:string;acceptance:string;deadlineHours:number;escalation:string;owner:string};
export type Catalog={nodes:Entity[];edges:Relation[];handoffs?:Handoff[]};
...
export type Assignment={id:string;personId:string;positionId:string;type:'主岗'|'兼岗'|'支援';start:string;end:string;reason:string;targetStars?:number};
export type Policy={id:string;scenarioId:string;name:string;roleId:string;requirements:{capabilityId:string;level:number;critical:boolean;behavior?:string}[];minSamples:number;allowGuided:boolean;version:string};
...
export type Assessment={id:string;personId:string;capabilityId:string;scenarioId:string;targetLevel:number;targetRoleId?:string;targetStar?:number;status:'草稿'|'待复核'|'已生效'|'已退回';submitter:string;reviewer:string;reason:string;factIds:string[];evidenceSnapshot?:Fact[];standardSnapshot?:Entity[];version:string;previousLevel:number|null;resultLevel:number|null;created:string;effective:string;reviewedAt?:string;supersedes?:string;roleBehaviorSnapshot?:string};
...
export type Development={id:string;personId:string;targetRoleId:string;track:'专业发展'|'管理发展';mentorId:string;months:number;wish:string;targetPolicyId:string;milestones:{name:string;done:boolean;evidence:string;systemReview?:boolean}[];status:'培养中'|'待复核'|'后备就绪';created:string;purpose?:'岗位成长'|'后备培养';targetConfirmed?:boolean;resources?:{resourceId:string;trained:boolean;note:string;legacyStatus?:string}[];assessmentIds?:string[];due?:string;targetVersion?:string;confirmations?:{date:string;reviewer:string;reason:string;signature:string}[]};
...
export function seedCatalog():Catalog{
 ...
 reference.sequences.forEach(s=>add(s.id,'sequence',s.name,'岗位职级办法修订稿第2—3页；岗位目录参考'));
 base.roles.forEach(r=>{add(r.id,'role',r.name,'职责边界与能力要求由关联任务和标准共同定义');edge(r.domainId,r.id,'domainRole');edge(reference.jobs.find(j=>j.id===reference.profiles.find(p=>p.roleId===r.id)?.jobId)?.sequenceId||'SEQ-02',r.id,'sequenceRole');r.capabilityIds.forEach(id=>edge(r.id,id,'roleCapability'));r.scenarioIds.forEach(id=>edge(r.id,id,'roleScenario'));r.taskIds.forEach(id=>edge(r.id,id,'roleTask'))});
 ...
 edges.filter(e=>e.type==='roleCapability').forEach(e=>{e.behaviors=[1,2,3,4,5].map(star=>base.capabilityTargets.find(t=>t.roleId===e.from&&t.capabilityId===e.to&&t.star===star)?.behavior||'')});
 return {nodes,edges};
}
export function createInitialState():State{
 ...
 const policies:Policy[]=base.scenarios.map(sc=>({id:`POL-${sc.id}`,scenarioId:sc.id,name:sc.name,roleId:sc.roleIds[0],requirements:unique(base.tasks.filter(t=>sc.taskIds.includes(t.id)).flatMap(t=>t.roleCapabilityMap[sc.roleIds[0]]||t.capabilityIds)).map(capabilityId=>({capabilityId,level:sc.riskLevel==='较高'?3:2,critical:capabilityId==='CAP-06',behavior:base.capabilityTargets.find(t=>t.roleId===sc.roleIds[0]&&t.capabilityId===capabilityId&&t.targetLevel===(sc.riskLevel==='较高'?3:2)&&t.behavior)?.behavior||''})),minSamples:2,allowGuided:sc.id!=='SCN-05',version:base.meta.standardVersion}));
 policies.push({id:'POL-EXPERT',scenarioId:'SCN-04',name:'高级故障研判保障',roleId:'ROLE-02',requirements:[{capabilityId:'CAP-04',level:5,critical:true},{capabilityId:'CAP-06',level:4,critical:true}],minSamples:2,allowGuided:false,version:base.meta.standardVersion});
 const facts:Fact[]=[];
 ...
 s.actions.push({id:'ACT-03',diagnosisId:'DIAG-01',category:'渠道版本不一致',title:'核验停电反馈知识在智能客服端的同步差异',ownerId:'DEMO-035',due:'2026-09-10',status:'待办理',deliverable:'',acceptance:'对照 KN-FAQ-002 的发布与渠道版本、同步批次，提交受影响记录和复验结果'});
 s.diagnoses[0].causes[0].detail='对照 KN-FAQ-002 发布K2.0与智能客服端K1.0的差异，核对服务当时实际使用的知识版本；不直接归因于个人能力';
...
export function checkCatalog(c:Catalog,policies:Policy[]){
 ...
 policies.forEach(p=>{if(!byId.get(p.scenarioId)?.active||!byId.get(p.roleId)?.active)errors.push(`${p.name}：场景或岗位已停用`);if(!p.requirements.length||p.minSamples<1)errors.push(`${p.name}：须设置门槛与最小样本`);p.requirements.forEach(r=>{if(!byId.get(r.capabilityId)?.active||r.level<1||r.level>5)errors.push(`${p.name}：无效能力要求`)});const taskIds=c.edges.filter(e=>e.type==='scenarioTask'&&e.from===p.scenarioId).map(e=>e.to);const caps=unique(c.edges.filter(e=>{if(e.type!=='taskCapability'||!taskIds.includes(e.from))return false;const original=base.tasks.find(t=>t.id===e.from);return !original||!original.capabilityIds.includes(e.to)||(original.roleCapabilityMap[p.roleId]||[]).includes(e.to)}).map(e=>e.to));if(p.id!=='POL-EXPERT'&&caps.some(id=>!p.requirements.some(r=>r.capabilityId===id)))errors.push(`${p.name}：新增任务能力尚未纳入场景门槛`)});
 c.edges.filter(e=>e.type==='roleCapability').forEach(e=>{if(e.targetLevels&&(e.targetLevels.length!==5||e.targetLevels.some(l=>!Number.isInteger(l)||l<1||l>5)))errors.push(`${e.from} → ${e.to}：目标等级必须是五个 L1—L5 整数`);if(e.requirements&&(e.requirements.length!==5||e.requirements.some(r=>typeof r.required!=='boolean'||!r.thresholdType.trim())))errors.push(`${e.from} → ${e.to}：必选标志与门槛类型须逐星级完整填写`)});
 errors.push(...checkHandoffs(c));
 return unique(errors);
}
export function catalogChanges(a:Catalog,b:Catalog){const entries:{id:string;name:string;change:string;before:string;after:string}[]=[];b.nodes.forEach(n=>{const old=a.nodes.find(x=>x.id===n.id);if(!old)entries.push({id:n.id,name:n.name,change:'新增',before:'—',after:n.description});else if(JSON.stringify(old)!==JSON.stringify(n))entries.push({id:n.id,name:n.name,change:old.active&&!n.active?'停用':'修改',before:old.description,after:n.description})});b.edges.filter(e=>!a.edges.some(x=>x.id===e.id)).forEach(e=>entries.push({id:e.from,name:`${e.from} → ${e.to}`,change:'新增关系',before:'—',after:e.type}));a.edges.filter(e=>!b.edges.some(x=>x.id===e.id)).forEach(e=>entries.push({id:e.from,name:`${e.from} → ${e.to}`,change:'移除关系',before:e.type,after:'—'}));for(const e of b.edges){const old=a.edges.find(x=>x.id===e.id);if(old&&JSON.stringify(old)!==JSON.stringify(e))entries.push({id:e.from,name:`${e.from} → ${e.to}`,change:e.type==='roleCapability'?'修改岗位要求':'修改关系',before:JSON.stringify(old),after:JSON.stringify(e)})}for(const h of [...(a.handoffs||[]),...(b.handoffs||[])]){const old=a.handoffs?.find(x=>x.id===h.id),next=b.handoffs?.find(x=>x.id===h.id);if(JSON.stringify(old)!==JSON.stringify(next)&&!entries.some(e=>e.name===h.id))entries.push({id:h.taskId,name:h.id,change:old?next?'修改协同约定':'移除协同约定':'新增协同约定',before:old?JSON.stringify(old):'—',after:next?JSON.stringify(next):'—'})}return entries;}
export function impact(s:State,draft=s.draft){
 ...
 const changed=unique(catalogChanges(s.catalog,draft.catalog).filter(c=>!c.change.includes('协同')).map(c=>c.id));const changedPolicies=draft.policies.filter(p=>JSON.stringify(p)!==JSON.stringify(s.policies.find(x=>x.id===p.id))).map(p=>p.id);
 ...
 const changedHandoffs=[...(s.catalog.handoffs||[]),...(draft.catalog.handoffs||[])].filter(h=>JSON.stringify(s.catalog.handoffs?.find(x=>x.id===h.id))!==JSON.stringify(draft.catalog.handoffs?.find(x=>x.id===h.id)));
 const coreRoleIds=unique([...s.catalog.nodes,...draft.catalog.nodes].filter(n=>n.kind==='role'&&reached.has(n.id)).map(n=>n.id).concat(draft.policies.filter(p=>changedPolicies.includes(p.id)).map(p=>p.roleId)));
 const roleIds=unique(coreRoleIds.concat(changedHandoffs.flatMap(h=>[h.leadRoleId,...h.supportRoleIds,h.reviewRoleId,h.escalateRoleId])));
 const policyIds=unique(changedPolicies.concat(draft.policies.filter(p=>capIds.some(id=>p.requirements.some(r=>r.capabilityId===id))||reached.has(p.scenarioId)||coreRoleIds.includes(p.roleId)).map(p=>p.id)));
 const affectedCaps=unique(capIds.concat(draft.policies.filter(p=>policyIds.includes(p.id)||changedHandoffs.some(h=>h.scenarioId===p.scenarioId)).flatMap(p=>p.requirements.map(r=>r.capabilityId))));
...
export function publishDraft(s:State){
 ...
 const affects=impact(s);const version=`REF-BO-1.${s.versions.length}`;
...
export function submitAssessment(s:State,id:string){const a=s.assessments.find(a=>a.id===id)!;if(a.status!=='草稿'&&a.status!=='已退回')throw new Error('当前状态不可重复提交');const policy=s.policies.find(p=>p.scenarioId===a.scenarioId);const c=evidenceCheck(s,a.personId,a.capabilityId,a.scenarioId,policy?.minSamples||2);if(!c.ok||c.level===null)throw new Error(c.reasons.join('；')||'证据不足');let roleBehavior=policy?.requirements.find(r=>r.capabilityId===a.capabilityId&&r.level===a.targetLevel)?.behavior||s.catalog.nodes.find(n=>n.active&&n.level===a.targetLevel&&s.catalog.edges.some(e=>e.type==='capabilityBehavior'&&e.from===a.capabilityId&&e.to===n.id))?.description||'';
 if(a.targetRoleId&&a.targetStar){
  const edge=s.catalog.edges.find(e=>e.type==='roleCapability'&&e.from===a.targetRoleId&&e.to===a.capabilityId);
  const original=base.capabilityTargets.find(t=>t.roleId===a.targetRoleId&&t.capabilityId===a.capabilityId&&t.star===a.targetStar);
  const level=edge?.targetLevels?.[a.targetStar-1]??original?.targetLevel;
  if(!Number.isInteger(a.targetStar)||a.targetStar<1||a.targetStar>5||!edge||level!==a.targetLevel||![a.targetRoleId,a.capabilityId].every(id=>s.catalog.nodes.some(n=>n.id===id&&n.active)))throw new Error('岗位目标要求已变化，请重新确认本次评价目标');
  roleBehavior=edge.behaviors?.[a.targetStar-1]??original?.behavior??'';
 }
 a.factIds=c.valid.map(f=>f.id);a.evidenceSnapshot=structuredClone(c.valid);a.standardSnapshot=structuredClone(s.catalog.nodes.filter(n=>n.id===a.capabilityId||s.catalog.edges.some(e=>e.from===a.capabilityId&&e.to===n.id&&e.type==='capabilityBehavior')));a.roleBehaviorSnapshot=roleBehavior;a.resultLevel=c.level;a.status='待复核';a.version=currentVersion(s);a.reason='';addAudit(s,'提交评价',`${nameOf(s,a.personId)} / ${nameOf(s,a.capabilityId)}`);}
...
export function syncLegacyModel(s:State){
 ...
 db.capabilityTargets.splice(0,db.capabilityTargets.length,...base.capabilityTargets.filter(t=>rs.some(r=>r.id===t.roleId&&r.capabilityIds.includes(t.capabilityId))).map(t=>({...t,behavior:edges.find(e=>e.type==='roleCapability'&&e.from===t.roleId&&e.to===t.capabilityId)?.behaviors?.[t.star-1]??t.behavior,targetLevel:edges.find(e=>e.type==='roleCapability'&&e.from===t.roleId&&e.to===t.capabilityId)?.targetLevels?.[t.star-1]??t.targetLevel,required:edges.find(e=>e.type==='roleCapability'&&e.from===t.roleId&&e.to===t.capabilityId)?.requirements?.[t.star-1]?.required??t.required,thresholdType:edges.find(e=>e.type==='roleCapability'&&e.from===t.roleId&&e.to===t.capabilityId)?.requirements?.[t.star-1]?.thresholdType||t.thresholdType,standardVersion:currentVersion(s)})));
 rs.forEach(r=>r.capabilityIds.forEach(id=>{if(!db.capabilityTargets.some(t=>t.roleId===r.id&&t.capabilityId===id))for(let star=1;star<=5;star++)db.capabilityTargets.push({...meta,id:`REQ-${r.id}-${id}-${star}`,roleId:r.id,star,capabilityId:id,behavior:edges.find(e=>e.type==='roleCapability'&&e.from===r.id&&e.to===id)?.behaviors?.[star-1]||'',targetLevel:edges.find(e=>e.type==='roleCapability'&&e.from===r.id&&e.to===id)!.targetLevels![star-1],required:edges.find(e=>e.type==='roleCapability'&&e.from===r.id&&e.to===id)?.requirements?.[star-1]?.required??true,thresholdType:edges.find(e=>e.type==='roleCapability'&&e.from===r.id&&e.to===id)?.requirements?.[star-1]?.thresholdType||'新增能力演示要求',scenarioIds:r.scenarioIds})}));
 ...
 s.assessments.filter(a=>a.status==='已生效').forEach(a=>{const fs=a.evidenceSnapshot||s.facts.filter(f=>a.factIds.includes(f.id));db.evidence.push({...base.evidence[0],id:a.id,name:`${nameOf(s,a.personId)} · 已复核实操证据`,employeeId:a.personId,capabilityIds:[a.capabilityId],scenarioIds:[a.scenarioId],sourceRecordId:a.factIds.join(' / '),observedAt:fs.map(f=>f.observed).sort().at(-1)||a.created,validUntil:fs.map(f=>f.expires).sort()[0]||s.date,summary:a.reason,evidenceStatus:fs.some(f=>f.expires<s.date)?'expired':'admissible',standardVersion:a.version,observations:[{roleBehavior:a.roleBehaviorSnapshot||'',capabilityId:a.capabilityId,level:a.resultLevel!,behaviorId:s.catalog.edges.filter(e=>e.from===a.capabilityId).map(e=>s.catalog.nodes.find(n=>n.id===e.to)).find(n=>n?.level===a.resultLevel)?.id||'',result:'人工复核通过（模拟）'}]})});
 db.meta.standardVersion=currentVersion(s);
}

export function checkHandoffs(c:Catalog){const errors:string[]=[];const keys=new Set<string>(),ids=new Set<string>();for(const h of c.handoffs||[]){const key=`${h.scenarioId}:${h.taskId}`;if(keys.has(key)||ids.has(h.id))errors.push(`${h.id}：场景任务或记录编码重复`);keys.add(key);ids.add(h.id);const active=(id:string,kind:string)=>c.nodes.some(n=>n.id===id&&n.kind===kind&&n.active);if(!active(h.scenarioId,'scenario')||!active(h.taskId,'task')||!c.edges.some(e=>e.type==='scenarioTask'&&e.from===h.scenarioId&&e.to===h.taskId))errors.push(`${h.id}：场景任务关系不存在或已停用`);if(!c.edges.some(e=>e.type==='roleScenario'&&e.from===h.leadRoleId&&e.to===h.scenarioId)||!c.edges.some(e=>e.type==='roleTask'&&e.from===h.leadRoleId&&e.to===h.taskId))errors.push(`${h.id}：主办岗位未承担该场景与任务，请先完善业务关系`);if(![h.leadRoleId,...h.supportRoleIds,h.reviewRoleId,h.escalateRoleId].every(id=>active(id,'role')))errors.push(`${h.id}：主办、协办、复核或升级岗位无效`);if(h.supportRoleIds.includes(h.leadRoleId)||new Set(h.supportRoleIds).size!==h.supportRoleIds.length||h.reviewRoleId===h.leadRoleId)errors.push(`${h.id}：主办不能兼作协办或本任务复核`);if(!h.materials.trim()||!h.acceptance.trim()||!h.escalation.trim()||!h.owner.trim()||!Number.isFinite(h.deadlineHours)||h.deadlineHours<=0)errors.push(`${h.id}：请补齐交接材料、接收条件、时限、升级条件和责任专业`)}return errors}

export function roleGrowthTarget(s:State,personId:string,roleId:string):number|null{
 const assignments=activeAssignments(s).filter(a=>a.personId===personId&&s.positions.find(p=>p.id===a.positionId)?.roleId===roleId);
 if(!assignments.length)return null;
 const explicit=assignments.find(a=>Number.isInteger(a.targetStars)&&a.targetStars!>=1&&a.targetStars!<=5)?.targetStars;
 if(explicit!==undefined)return explicit;
 const person=personAt(s,personId);return person.roleId===roleId?person.targetStars:null;
}
