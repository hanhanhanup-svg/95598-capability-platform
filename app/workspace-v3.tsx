...
import {Network,LayoutDashboard,BriefcaseBusiness,TrendingUp,ShieldCheck,Settings2,ClipboardCheck,BookOpen,ChevronDown,X,ArrowUpRight,Users} from 'lucide-react';
import {Sidebar,SidebarProvider,SidebarContent,SidebarHeader,SidebarFooter,SidebarMenu,SidebarMenuItem,SidebarMenuButton,SidebarMenuSub,SidebarMenuSubItem,SidebarMenuSubButton,SidebarTrigger,useSidebar} from '@/components/ui/sidebar';
...
export const groups=[
 ...
 {id:'wall',label:'岗位能力墙',icon:BriefcaseBusiness,pages:[['graph','岗位图谱'],['wall','人员能力分析'],['matrix','组织能力分析'],['business','岗位与业务标准'],['handoff','协同责任']]},
 {id:'governance',label:'标准管理',icon:BookOpen,pages:[['governance','业务标准管理']]},
 {id:'supply',label:'业务保障',icon:Users,pages:[['supply','能力供需与预警'],['diagnosis','业务结果诊断']]},
 {id:'evaluation',label:'评价与授权',icon:ClipboardCheck,pages:[['evaluation','能力评价'],['authorization','上岗授权']]},
 {id:'talent',label:'培养发展',icon:TrendingUp,pages:[['talent','统一培养计划']]},
 {id:'organization',label:'体系管理',icon:Settings2,pages:[['organization','组织与岗位'],['templates','模板与批量维护'],['handoff-admin','协同标准维护'],['data','数据治理']]}
];
const alias=(id:string)=>id==='growth'?'talent':id;
const validPage=(id:string)=>groups.some(g=>g.pages.some(p=>p[0]===alias(id)));
function WorkspaceNavigation({nav,groupId,page,onNavigate}:{nav:typeof groups;groupId:string;page:string;onNavigate:(id:string)=>void}){
 const {setOpenMobile}=useSidebar();
 const navigate=(id:string)=>{onNavigate(id);setOpenMobile(false)};
 return <SidebarMenu className="nav-menu">{nav.map(g=><SidebarMenuItem key={g.id}>
  <SidebarMenuButton className="nav-link" isActive={groupId===g.id} onClick={()=>navigate(g.id)}>
   <g.icon size={19}/><span>{g.label}</span>
   {g.id==='wall'?<ChevronDown size={15} className={`nav-group-chevron ${groupId==='wall'?'expanded':''}`}/>:groupId===g.id&&<span className="nav-dot"/>}
  </SidebarMenuButton>
  {g.id==='wall'&&groupId==='wall'&&<SidebarMenuSub className="wall-subnav" aria-label="岗位能力墙二级菜单">
   {g.pages.map(([id,label])=><SidebarMenuSubItem key={id} className={id==='business'?'wall-subnav-section':undefined}>
    <SidebarMenuSubButton className="nav-sub-link" href={`#${id}`} isActive={page===id} aria-current={page===id?'page':undefined} onClick={e=>{if(e.ctrlKey||e.metaKey||e.shiftKey||e.altKey)return;e.preventDefault();navigate(id)}}>
     <span className="nav-sub-dot"/><span>{label}</span>
...
function Workspace(){
 ...
 const compactHeading=page==='graph'||page==='wall';
 return <SidebarProvider style={{'--sidebar-width':'218px'} as CSSProperties}><Sidebar className="app-sidebar"><SidebarHeader className="brand"><span className="brand-mark"><Network size={27}/></span><div><strong>95598</strong><span>岗位能力图谱</span></div></SidebarHeader><SidebarContent><div className="nav-caption">工作空间</div><WorkspaceNavigation nav={nav} groupId={group.id} page={page} onNavigate={go}/><div className="sidebar-note"><span>岗位能力墙</span><p>看清现状<br/>明确差距<br/>落实成长行动</p></div></SidebarContent><SidebarFooter className="sidebar-bottom"><button className="version" onClick={()=>setAbout(true)}><i className="live-dot"/>演示工作空间 <span>V3.0</span></button><div className="user-row"><span className="avatar">{viewRole.slice(0,1)}</span><div><b>{viewRole}视角</b><small>95598 客户服务中心</small></div><ShieldCheck size={17}/></div></SidebarFooter></Sidebar>
 <div className="app-shell"><main className="workspace">
 {page==='overview'?<div className="v3-overview-mobile-nav"><SidebarTrigger className="sidebar-toggle" aria-label="切换导航菜单"/></div>:<header className={`page-heading v3-workspace-header${compactHeading?' v3-workspace-header-compact':''}`}>
  <div className="v3-heading-title">
   <SidebarTrigger className="sidebar-toggle" aria-label="切换导航菜单"/>
   <h1>{group.label}</h1>
   <button className="mock-badge v3-demo-badge" aria-label="查看模拟数据说明" onClick={()=>setAbout(true)}>模拟数据</button>
  </div>
  {!compactHeading&&<div className="v3-heading-controls">
   <div className="v3-view-controls">
    <span>演示视角</span>
    <Picker label="切换工作视角" value={viewRole} onChange={v=>{setViewRole(v as ViewRole);go('overview')}} options={(['管理者','班组长','员工','评价人员','体系管理员'] as ViewRole[]).map(v=>({value:v,label:v}))}/>
    {['员工','班组长','评价人员'].includes(viewRole)&&<Picker label="当前演示人员" value={viewerId} onChange={id=>{setViewerId(id);go('overview',{personId:id})}} options={base.people.map(p=>({value:p.id,label:`${p.name} · ${personAt(s,p.id).team}`}))}/>}
   </div>
   <Field label="观察日期" type="date" min={s.versions[0].date} value={s.date} onChange={e=>mutate(st=>setReferenceDate(st,e.target.value))}/>
   <Badge tone="gray">{currentVersion(s)}</Badge>
  </div>}
  <button className="button outline v3-heading-return" onClick={()=>go(group.id==='wall'?'overview':'wall')}>{group.id==='wall'?'返回工作台':'查看岗位能力墙'}<ArrowUpRight size={15}/></button>
 </header>}
 ...
 {group.id!=='wall'&&group.pages.length>1&&<div className="v3-section-tabs"><Segments value={page} onChange={id=>go(id,group.id==='evaluation'?{personId}:{})} options={group.pages.map(([value,label])=>({value,label}))}/></div>}
 {context.recordId&&group.id!=='evaluation'&&<div className="v3-record-context"><span>当前办理：{nameOf(s,personId)}{context.scenarioId?` · ${nameOf(s,context.scenarioId)}`:''}</span><small>{context.recordId}</small><button className="text-button" onClick={()=>go(page)}>清除事项定位</button></div>}
 {page==='overview'&&<WorkbenchView viewRole={viewRole} viewerId={viewerId} onOpen={go} onDetail={openDetail}/>}
 ...
 {page==='handoff-admin'&&<><HandoffView editable/><div className="mg-inset">{s.draft?<button className="button primary" onClick={()=>go('governance',{tab:'release'})}>继续办理本次修订</button>:<span className="muted">暂无待办理修订</span>}</div></>}
