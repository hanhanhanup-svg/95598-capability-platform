...
export const createWorkspaceState=()=>upgradeWorkspace(createInitialState());
...
export function todos(s:State):Todo[]{
 ...
 s.actions.filter(a=>a.status!=='已完成').forEach(a=>push({id:`action:${a.id}`,title:a.title,category:a.category,actor:a.status==='待验收'?'管理者':'班组长',ownerId:a.status==='待验收'?'':a.ownerId,ownerLabel:a.status==='待验收'?'改善验收负责人':nameOf(s,a.ownerId),due:a.due,page:'diagnosis',context:{recordId:a.id,personId:a.ownerId,tab:'all'},step:a.status==='待验收'?'检查产出并验收':'提交改善材料'}));
...
export function roleRequirements(c:Catalog,roleId:string,star:number){return c.edges.filter(e=>e.type==='roleCapability'&&e.from===roleId&&c.nodes.some(n=>n.id===e.to&&n.active)).map(e=>{const old=base.capabilityTargets.find(t=>t.roleId===roleId&&t.capabilityId===e.to&&t.star===star);return {capabilityId:e.to,level:e.targetLevels?.[star-1]??old?.targetLevel??null,required:e.requirements?.[star-1]?.required??old?.required??true,thresholdType:e.requirements?.[star-1]?.thresholdType||old?.thresholdType||'新增能力演示要求'}})}
